  ...
  <v-app>
    <v-main>
      <v-toolbar>
        <v-toolbar-title>
          <span>HeartRate</span>
          <span v-if="!$vuetify.breakpoint.smAndDown" class="font-weight-light ml-1">Graph</span>
        </v-toolbar-title>
        <v-btn class="ml-4" rounded text :disabled="!isBluetoothAvailable()" @click="connect">
          Connect
        </v-btn>
        <v-spacer></v-spacer>
        ...
        <v-btn v-if="!$vuetify.breakpoint.smAndDown" rounded text href="https://github.com/henkelmax/heartrate-graph"
          target="_blank">
      ...
      <v-row v-if="chartData.length <= 1" justify="center" class="ma-2">
        <v-col xl="6" lg="6" md="6" sm="12" cols="12">
          <v-alert v-if="isBluetoothAvailable()" border="top" colored-border type="info" elevation="2" cols="6">
      ...
      <GChart v-else class="chart mt-4" type="LineChart" :data="chartData" :options="chartOptions" fill-height />
